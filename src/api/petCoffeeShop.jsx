...
export const getAllShopsDistance = async (latitude, longitude) => {
  const response = await api.get(`/api/v1/petcoffeeshops?Latitude=${latitude}&Longitude=${longitude}`);
  return response.data;
};

export const getAllPetCoffeeShops = async () => {
  const response = await api.get(`/api/v1/petcoffeeshops`);
  return response.data;
};
...
export const getAllPopularShopsDistance = async (latitude, longitude) => {
  const response = await api.get(`/api/v1/petcoffeeshops/popular?Latitude=${latitude}&Longitude=${longitude}`);
  return response.data;
};
...
export const searchPetCoffeeShop = async (
  searchQuery,
  pageNumber,
  pageSize,
) => {
  const response = await api.get(
    `/api/v1/petcoffeeshops?Search=${searchQuery}&PageNumber=${pageNumber}&PageSize=${pageSize}`,
  );
  return response.data;
};
