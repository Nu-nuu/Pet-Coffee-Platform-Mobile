...
export const getPostShop = async (id) => {
    const response = await api.get(`/api/v1/shops/posts?ShopId=${id}`);
    return response.data;
};

export const getPostTagShop = async (id) => {
    const response = await api.get(`/api/v1/posts/tagged/${id}`);
    return response.data;
};

export const updatePost = async data => {
    const response = await api.put(`/api/v1/posts`, data
        , {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
...
export const deletePost = async (id) => {
    const response = await api.delete(`/api/v1/posts/${id}`, id);
    return response.data;
};