...
export const getAllPopularShopsDistanceThunk = createAsyncThunk(
  ...
  async ({latitude, longitude}, thunkAPI) => {
    try {
      const response = await getAllPopularShopsDistance(latitude, longitude);
...
export const getPetCoffeeShopDetailThunk = createAsyncThunk(
  ...
  async ({id, latitude, longitude}, thunkAPI) => {
    try {
      const response = await getPetCoffeeShopDetail({id, latitude, longitude});
      return response;
    } catch (error) {
      return thunkAPI.rejectWithValue(error.response.data);
    }
...
export const searchPetCoffeeShopsThunk = createAsyncThunk(
  'petCoffeeShop/searchPetCoffeeShops',
  async ({searchQuery, pageNumber, pageSize}, thunkAPI) => {
    try {
      const response = await searchPetCoffeeShop(
        searchQuery,
        pageNumber,
        pageSize,
      );
